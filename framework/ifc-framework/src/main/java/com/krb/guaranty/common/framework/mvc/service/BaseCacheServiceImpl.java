...
public abstract class BaseCacheServiceImpl<M extends BaseRepository4RDB<T>, T extends GenericDomain> extends BaseServiceImpl<M, T> implements BaseService<T> {
    ...
    public boolean updateBatchById(Collection<T> entityList, int batchSize) {
        List<String> cacheKeys = new ArrayList<>(entityList.size());
        for (T entity : entityList) {
            cacheKeys.add(getCacheKey(getId(entity)));
        }
